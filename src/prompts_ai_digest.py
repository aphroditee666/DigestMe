...
{{
  "index": 序号(整数),
  "category": "所属分类（{" / ".join(CATEGORIES.keys())}）",
  "subtype": "子类型（学术论文 / 技术/算法 / 产品/应用）",
...
子类型（subtype）定义：
- 学术论文：来自arXiv、顶会、期刊等学术渠道，内容侧重学术研究、实验验证、理论证明
- 技术/算法：内容侧重技术原理、算法设计、模型架构、训练方法等深度技术内容
- 产品/应用：内容侧重产品发布、应用案例、行业动态、工具使用、商业合作等应用层面内容

要求：
1. category 和 subtype 必须基于正文内容（不仅仅是标题）准确判断
2. 摘要150-400字，必须涵盖：背景/问题、核心方法/技术方案、关键结果/结论
3. 提供2-5个关键要点，每条15-50字，必须有实质信息（数字、方法名、技术名称等），切忌空洞概括
4. 如果文章提到具体模型名、数据集、性能指标、开源地址，务必写入摘要或要点
5. 用中文输出
